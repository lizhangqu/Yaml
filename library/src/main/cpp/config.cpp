...
namespace yaml {
    ...
    public:
        ...
        an<ConfigItem> Traverse(const std::string &key);
    ...
    class ConfigDataManager : public std::map<std::string, weak<ConfigData>> {
    public:
        an<ConfigData> GetConfigData(const std::string &config_file_path);
...
// ConfigMap members
    ...
    an<ConfigItem> ConfigMap::Get(const std::string &key) const {
    ...
    an<ConfigValue> ConfigMap::GetValue(const std::string &key) const {
        return As<ConfigValue>(Get(key));
    }

    bool ConfigMap::Set(const std::string &key, an<ConfigItem> element) {
        map_[key] = element;
        return true;
    }
...
// Config members
    ...
    an<ConfigItem> Config::GetItem(const std::string &key) {
        ALOGI("read: %s", key.c_str());
        return data_->Traverse(key);
    }

    an<ConfigValue> Config::GetValue(const std::string &key) {
        ALOGI("read: %s", key.c_str());
        return As<ConfigValue>(data_->Traverse(key));
    }

    an<ConfigList> Config::GetList(const std::string &key) {
        ALOGI("read: %s", key.c_str());
        return As<ConfigList>(data_->Traverse(key));
    }

    an<ConfigMap> Config::GetMap(const std::string &key) {
        ALOGI("read: %s", key.c_str());
    ...
    static size_t ResolveListIndex(an<ConfigItem> p, const std::string &key,
    ...
    bool Config::SetItem(const std::string &key, an<ConfigItem> item) {
...
// ConfigData members
    ...
    an<ConfigItem> ConfigData::Traverse(const std::string &key) {
